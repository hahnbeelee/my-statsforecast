...
$\text{Z}_2$ are obtained by minimising \$\_{t=1}^n \[Y_t -\_t()\]^2 \$.
However, an analytical solution to compute the $\text{Z}(\theta)$ was
obtained by H&B, which is given by

$$\text{Z}_t(\theta)=\theta Y_t +(1-\theta)(A_n +B_n t) \ \ t=1, \cdots n \tag 2 $$

where $\text{A}_n$ and $\text{B}_n$ are the minimum square coefficients
of a simple linear regression over $Y_1, \cdots Y_n$ against
$1, \cdots n \ $, given by
...
of are an ad-hoc combination (50%–50%) of the extrapolations of \$(0) \$
and $\text{Z}(2)$ by the linear regression model and the simple
exponential smoothing model respectively. We will refer to the above
setup as the standard Theta method (STheta).
...
\$\[Y\_{n+h}\|Y_1, , Y_n\]=\[1+(h-1)^2\]^2 \$ can be computed easily
from the state space model. Thus, the $(1-\alpha)\%$ prediction interval
for $Y_{n+h}$ is given by
...
![](AutoTheta_files/figure-markdown_strict/cell-8-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-9-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-12-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-19-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-22-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-26-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-31-output-1.png)
...
![](AutoTheta_files/figure-markdown_strict/cell-32-output-1.png)
